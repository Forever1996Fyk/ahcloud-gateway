package com.ahcloud.gateway.server.infrastructure.gateway.listener.nacos;
...
import com.ahcloud.gateway.server.infrastructure.gateway.listener.AbstractListDataChangedListener;
...
public class NacosDataChangeListener extends AbstractListDataChangedListener {
    ...
    public NacosDataChangeListener(final ConfigService configService) {
    ...
    protected String getConfig(String dataId) {
        String config;
        try {
            config = configService.getConfig(dataId, NacosPathConstants.GROUP, NacosPathConstants.DEFAULT_TIME_OUT);
