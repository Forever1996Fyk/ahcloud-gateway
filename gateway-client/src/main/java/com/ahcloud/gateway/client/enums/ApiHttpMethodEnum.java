...
import com.ahcloud.gateway.client.exception.GatewayClientException;
...
public enum ApiHttpMethodEnum {
    ...
    public static Integer getValueByName(final String name) {
        return Arrays.stream(ApiHttpMethodEnum.values())
                ...
                .map(item -> item.value)
                .orElseThrow(() -> new GatewayClientException(String.format(" this http method can not support %s", name)));
    }
